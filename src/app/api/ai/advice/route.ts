...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) return NextResponse.json({ error: "Ассистент временно недоступен" }, { status: 503 });
    const json = await response.json();
    const answer = json?.choices?.[0]?.message?.content;
    if (typeof answer !== "string" || !answer.trim()) throw new Error("Empty answer");
    return NextResponse.json({ answer: answer.trim() }, { headers: { "Cache-Control": "private, no-store" } });
  } catch {
    return NextResponse.json({ error: "Ассистент не ответил. Попробуйте ещё раз." }, { status: 503 });
  }
}
